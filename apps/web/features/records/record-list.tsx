...
export function RecordList(props: RecordListProps) {
  ...
  return <>
    ...
    <section className="table-shell" aria-label={`${noun} records`}><Table className="record-table"><thead><tr>{isStudent ? <><th>Name</th><th>LRN</th><th>Grade</th><th>Section</th><th>Status</th></> : <><th>Name</th><th>Employee number</th><th>Designation</th><th>Employment status</th></>}<th>Last updated</th><th><span className="sr-only">Actions</span></th></tr></thead>
      <tbody>{filtered.map((record) => <tr key={record.id} className="record-row" tabIndex={0} onClick={() => goToRecord(record.id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") goToRecord(record.id); }}><td className="primary-cell"><span className={`filing-tab ${isStudent ? gradeClasses[(record as Student).grade] : "tab-teacher"}`} /><strong>{record.name}</strong></td>{isStudent ? <><td className="tabular">{(record as Student).lrn}</td><td>{(record as Student).grade}</td><td>{(record as Student).section}</td><td><Badge variant="secondary">{(record as Student).status}</Badge></td></> : <><td className="tabular">{(record as Teacher).employeeNumber}</td><td>{(record as Teacher).designation}</td><td><Badge variant="outline">{(record as Teacher).status}</Badge></td></>}<td className="muted-cell">{record.updatedAt}</td><td><DropdownMenu><DropdownMenuTrigger render={<Button variant="ghost" size="icon" aria-label={`More actions for ${record.name}`}><Icon name="more" /></Button>} onClick={(event) => event.stopPropagation()} /><DropdownMenuContent align="end"><DropdownMenuGroup><DropdownMenuItem onClick={(event) => { event.stopPropagation(); goToRecord(record.id); }}>View record</DropdownMenuItem><DropdownMenuItem onClick={(event) => event.stopPropagation()}>Edit record</DropdownMenuItem></DropdownMenuGroup></DropdownMenuContent></DropdownMenu></td></tr>)}</tbody></Table>
