...
function Overview({ student }: { student: Student }) { return <div className="record-detail-layout"><div className="record-detail-content">
  <section className="info-section personal-information"><h2>Personal information</h2><dl className="name-breakdown"><div><dt>First name</dt><dd>{student.firstName}</dd></div><div><dt>Middle name</dt><dd>{student.middleName ?? "Not recorded"}</dd></div><div><dt>Last name</dt><dd>{student.lastName}</dd></div><div><dt>Suffix</dt><dd>{student.suffix ?? "Not recorded"}</dd></div></dl><dl className="personal-details-grid"><div><dt>Birthday</dt><dd>{student.birthday}</dd></div><div><dt>Birthplace</dt><dd>{student.birthplace}</dd></div><div><dt>Address</dt><dd>{student.address}</dd></div></dl></section>
...
</div><aside className="record-summary"><section><p className="record-kicker">Current enrollment</p><dl><div><dt>School year</dt><dd>2026–2027</dd></div><div><dt>Grade</dt><dd>{student.grade}</dd></div><div><dt>Section</dt><dd>{student.section}</dd></div><div><dt>Adviser</dt><dd>Maria Cruz</dd></div><div><dt>Status</dt><dd><span className="status status-active"><i />Active</span></dd></div></dl></section><section><p className="record-kicker">Record status</p><p className="summary-note">Last updated {student.updatedAt}. This view will warn you before a stale edit overwrites a newer record.</p></section></aside></div>; }
