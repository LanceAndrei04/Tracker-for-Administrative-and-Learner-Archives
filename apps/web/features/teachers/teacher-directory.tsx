...
import { LayoutGridIcon, ListIcon, PencilIcon, PlusIcon, SearchIcon } from "lucide-react";
...
function TeacherCards({ teachers, loading }: { teachers: TeacherDirectoryRecord[]; loading: boolean }) {
  ...
  return <section className="teacher-card-grid" aria-label="Teacher cards">{teachers.map((teacher) => <Card className="teacher-card" key={teacher.id}><Link href={`/teachers/${teacher.id}`} className="teacher-card-link" aria-label={`Open ${displayName(teacher)}'s record`}><CardContent className="teacher-card-photo-wrap"><Avatar className="teacher-photo" size="lg"><AvatarFallback>{initials(teacher)}</AvatarFallback></Avatar></CardContent><CardHeader><CardTitle>{displayName(teacher)}</CardTitle><p className="text-sm text-muted-foreground">{teacher.designation}</p></CardHeader></Link><CardFooter><span className="teacher-card-employee tabular">{teacher.employeeNumber}</span><Button nativeButton={false} variant="outline" size="sm" render={<Link href={`/teachers/${teacher.id}/edit`} />}><PencilIcon data-icon="inline-start" />Edit</Button></CardFooter></Card>)}</section>;
}

function TeacherTable({ teachers, loading, onOpen }: { teachers: TeacherDirectoryRecord[]; loading: boolean; onOpen: (id: string) => void }) {
  return <section className="table-shell" aria-label="Teacher records"><Table><TableHeader><TableRow><TableHead>Teacher</TableHead><TableHead>Position</TableHead><TableHead>Employee no.</TableHead><TableHead>Station status</TableHead><TableHead>Last updated</TableHead><TableHead><span className="sr-only">Actions</span></TableHead></TableRow></TableHeader><TableBody>{loading ? Array.from({ length: 5 }, (_, index) => <TableRow key={index}><TableCell colSpan={6}><Skeleton className="h-5 w-full" /></TableCell></TableRow>) : teachers.map((teacher) => <TableRow key={teacher.id} className="teacher-table-row" tabIndex={0} role="link" onClick={() => onOpen(teacher.id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onOpen(teacher.id); } }}><TableCell><div className="teacher-table-person"><Avatar className="teacher-table-photo" size="sm"><AvatarFallback>{initials(teacher)}</AvatarFallback></Avatar><strong>{displayName(teacher)}</strong></div></TableCell><TableCell>{teacher.designation}</TableCell><TableCell className="tabular">{teacher.employeeNumber}</TableCell><TableCell>{teacher.stationStatus ? <Badge variant="outline">{displayStationStatus(teacher.stationStatus)}</Badge> : "—"}</TableCell><TableCell className="text-muted-foreground">{formatDate(teacher.updatedAt)}</TableCell><TableCell><Button nativeButton={false} variant="ghost" size="sm" onClick={(event) => event.stopPropagation()} render={<Link href={`/teachers/${teacher.id}/edit`} />}><PencilIcon data-icon="inline-start" />Edit</Button></TableCell></TableRow>)}</TableBody></Table></section>;
}
